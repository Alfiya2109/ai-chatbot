...
const ExcelListTab = ({ uploadedFiles, renderTable, excelModalOpen, setExcelModalOpen, excelForm, setExcelForm, knowledgeBases, excelFormError, handleExcelModalSubmit, fetchKnowledgeBases, onBulkDelete, isLoading }) => {
  ...
  // Filter and sort Excel files based on search term and sort settings
  const filteredAndSortedFiles = useMemo(() => {
    ...
    return filtered;
  }, [uploadedFiles, searchTerm, selectedKBFilters, showSearchBox, sortField, sortDirection, selectedFileNameFilter, selectedDescriptionFilter, selectedAddedByFilter, selectedDateFilter]);
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <table className="min-w-full text-sm">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {/* Render table rows using filteredFiles */}
            {isLoading ? (
              <tr>
            ...
            <form onSubmit={handleExcelModalSubmit} className="space-y-4 text-sm">
              ...
              <div>
                ...
                <Select
                  ...
                  value={knowledgeBases.filter(kb => excelForm.knowledge_bases.includes(String(kb.id))).map(kb => ({ value: String(kb.id), label: kb.name }))}

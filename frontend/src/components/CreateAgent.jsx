        ...
        return (
          ...
          >
            <p className="text-gray-600 mb-4" style={{ fontSize: '1.2rem' }}>
              Drag & drop {activeTab === 'Files' ? 'files' : 'Excel/CSV files'}, or{' '}
              <span
                className="text-blue-600 cursor-pointer underline"
                onClick={() => document.getElementById(activeTab === 'Files' ? 'fileInput' : 'excelInput').click()}
              >
                click here
              </span>
            </p>
            <input
              ...
              accept={activeTab === 'Excel/CSV' ? '.csv, .xls, .xlsx' : undefined}
              className="hidden"
              id={activeTab === 'Files' ? 'fileInput' : 'excelInput'}
              style={{ display: 'none' }}
            />
            <p className="text-sm text-gray-500">
              Supported File Types: {activeTab === 'Files' ? '.pdf, .doc, .docx, .txt' : '.csv, .xls, .xlsx'}
            </p>
          ...
          <div className="w-full mt-4 flex flex-col items-center">
            ...
            <form className="w-full max-w-md space-y-4">
              <div>
                ...
                <input type="text" id="question" className="mt-1 block w-full p-2 border rounded-md" />
              </div>
              <div>
                <label htmlFor="answer" className="block text-sm font-medium text-gray-700">Answer</label>
                <textarea id="answer" className="mt-1 block w-full p-2 border rounded-md h-24" />
              </div>
              <div>
                <label htmlFor="category" className="block text-sm font-medium text-gray-700">Category</label>
                <input type="text" id="category" className="mt-1 block w-full p-2 border rounded-md" />
              </div>
              <div>
                <label htmlFor="subCategory" className="block text-sm font-medium text-gray-700">Sub Category</label>
                <input type="text" id="subCategory" className="mt-1 block w-full p-2 border rounded-md" />
              </div>
            </form>
          </div>
        );
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      ...
      <div className="w-full flex flex-col p-6 justify-center items-center">
        {renderContent()}
        {['Files', 'Text', 'Excel/CSV', 'Q&A'].includes(activeTab) && (
